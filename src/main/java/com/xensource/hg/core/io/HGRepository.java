...
public class HGRepository {
	...
	try {
	    ...
	    if (!myUpdateRepo) {
		if (cmd.equals("head")) {
		    linesList = eu.exec("hg tip", env_vars, hg_logdir, log);
		} else if (cmd.startsWith("log")) {
		    linesList = eu.exec("hg " + cmd, env_vars, hg_logdir, log);
		} else {
		...
		linesList = eu.exec("hg clone " + myRepositoryRoot, env_vars, basedir, log);
	    } else {
		if (cmd.equals("head")) {
		    ...
		    log.info("cmd = " + cmd);
		    linesList = eu.exec("hg pull -u", env_vars, hg_logdir, log);
		}
	    }
	...
	try {
	    ...
	    HGRepository currentRepo = new HGRepository(HGRepositoryLocation.parseURL("http://hg.hq.xensource.com/mirror/carbon/carbon/empty.hg"), "/usr/groups/xen/HG/mirror/carbon/carbon", false);
